        ...
        if (contents != null && contents.Count > 0)
        {
            foreach (Ektron.Cms.ContentData contentData in contents)
            {
                try
                {
                    ...
                    string teaser = content.Substring(0, 450).TrimEnd() + "..."; //xnList[0]["teaser"].InnerXml;
                    ...
                    DTSource.Rows.Add(title, subtitle, imgSliderSRC, iconSmallGrey, iconSmallRed, iconSmallWhite, headerIconSmallWhite, hrefId, url, content, teaser, quotesByName, quotesByOrganization, statement, tagline, counter);
                }
                catch (Exception ex)
                {
                    Log.WriteError(ex);
