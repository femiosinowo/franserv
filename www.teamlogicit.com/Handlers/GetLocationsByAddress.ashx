...
public class GetLocationsByAddress : IHttpHandler
{
    ...
    private string GetLocations(bool isInitialLoad, string address, string distance)
    {
        ...
        if (isInitialLoad)
        {
            ...
            {
                //also include the Canada locations as per Client request.
                //as per client hide the following location from the page
                if ((fd.Country == address || fd.Country == "CA") &&
                    fd.FransId.ToLower() != "arlingtonva999" &&
                    fd.FransId.ToLower() != "missionviejoca002")
                {
    ...
    private List<FransData> GetUserSelectedRaduisLocations(List<FransData> allLocations, string distance, double givLat, double givLon)
    {
        ...
        {
            var fransData = allLocations[i];
            var radlat1 = Math.PI * givLat / 180;
            var radlat2 = Math.PI * double.Parse(fransData.Latitude) / 180;
            var radlon1 = Math.PI * givLon / 180;
            var radlon2 = Math.PI * double.Parse(fransData.Longitude) / 180;
            var theta = givLon - double.Parse(fransData.Longitude);
            ...
            {
                //as per client hide the following location from the page
                if ((!dataList.Contains(fransData)) && 
                    fransData.FransId.ToLower() != "arlingtonva999" && 
                    fransData.FransId.ToLower() != "missionviejoca002")
                {
                    fransData.Miles = Math.Round(dist, 2);
                    dataList.Add(fransData);
                }
            }
        }
        return dataList;
