  ...
  $(document.body).on('click', '.desktop-nav-link.no-hover > a' ,function() {
    ...
    **********************************************************/
    ...
    if (fancybox.length > 0) {
        ...
        //Request Consultation (Local)

        $("#requestconsultation_lb, #minimenu #requestconsultation_lb").fancybox({
            'type': 'inline',
            'autoScale': true,
            'autoDimensions': true,
            'showNavArrows': false,
            'afterLoad': function () {
                $('.fancybox-skin').attr('id', 'requestconsultation-fancybox'); /*Add ID to .fancybox-skin for custom styling*/
            }
