...
//$("body").on("parallaxCodeReady", function () {
		//var timedParalax = setTimeout(function () {
    ...
    ImageScroll = function (imageHolder, options) {
        return {
            init: function () {
				...
				$("body").trigger("parallaxCodeReady");
            },
            ...
            _bindEvents: function () {
                ...
                if(this.settings.parallax === true) {
                    $win.on('scroll', function (evt) {
					  ...
					  //this is for partners & management parallax- see scripts.js
					  $('body').on('executeManualParallax', function (evt) {
                        self.scrollingState.holderDistanceFromTop = self.$imageHolder.offset().top - $win.scrollTop();
                        self._requestTick();
                    });
                }
