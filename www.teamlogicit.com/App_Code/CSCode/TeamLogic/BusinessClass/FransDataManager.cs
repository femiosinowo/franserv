...
namespace TeamLogic.CMS
{
    ...
    public class FransDataManager
    {
        ...
        private static string LocalFransCacheKey = "LocalFransSite";
        ...
        public static Testimonials GetTestimonials(Testimonials localTestimonialsData)
        {
            ...
            if (!string.IsNullOrEmpty(data))
            {
                ...
                if (testimonials != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(testimonials.EmailAddress))
                        responseData.EmailAddress = testimonials.EmailAddress;
                    if (!string.IsNullOrEmpty(testimonials.PhoneNumber))
                        responseData.PhoneNumber = testimonials.PhoneNumber;
                }
            }
        ...
        public static SocialMedia GetFransSocialMediaLinks(SocialMedia nationalSMData)
        {
            ...
            if (localSocialLinks != null)
            {
                ...
                responseData.ITInflectionsImgPath = nationalSMData.ITInflectionsImgPath;
                ...
                //ITInflectionsUrl
                responseData.ITInflectionsUrl = localSocialLinks.ITInflectionsUrl;
                //StumbleUpon
        ...
        public static FransWorkareaData GetFransWorkareaData(string fransId = "", bool refresh = false)
        {
            ...
            if (!string.IsNullOrEmpty(fransId))
            {
                try
                {
                    string cacheKey = "SirSpeedy-FransWorkareaData" + fransId;
                                ...
                                if (reader != null && reader.HasRows)
                                {
                                    while (reader.Read())
                                    {
                                        fransWorkAreaData = new FransWorkareaData()
                                        {
                                            ...
                                            ManageITContentIds = reader["Center_ManageITServices_Ids"] is DBNull ? null : GetIdsFromString(reader["Center_ManageITServices_Ids"].ToString()),
                                            ITSolutionContentIds = reader["Center_ITSolution_Ids"] is DBNull ? null : GetIdsFromString(reader["Center_ITSolution_Ids"].ToString()),
                                            ProjectExpertsContentIds = reader["Center_ProjectExperts_Ids"] is DBNull ? null : GetIdsFromString(reader["Center_ProjectExperts_Ids"].ToString()),
                                            ITOurSolutionContentIds = reader["Center_ITOurSolution_Ids"] is DBNull ? null : GetIdsFromString(reader["Center_ITOurSolution_Ids"].ToString())
                                        };
                                    }
                                }
                            }
        ...
        public static FransThirdPartyData GetFransThirdPartyData(string centerId = "", bool refresh = false)
        {
            ...
            if (!string.IsNullOrEmpty(fransId))
            {
                try
                {
                    string cacheKey = "SirSpeedy-FransThirdPartyData" + fransId;
                    bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static FransM3TData GetFransM3TData(string fransId = "", bool refresh = false)
        {
            ...
            if (!string.IsNullOrEmpty(fransId))
            {
                try
                {
                    string cacheKey = "SirSpeedy-FransM3TData" + fransId;
                    bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        /// </summary>       
        public static void SaveRequestToQuoteData(string centerId, string firstName, string lastName, string jobTitle, string companyName, string email, string phoneNumber, string projectName, string projectDescription, string projectBudget, string ektronUploadedFilesIds)
        {
            try
            {
                //send email
                SendRequestToQuoteInfoToCenter(centerId, firstName, lastName, jobTitle, companyName, email, phoneNumber, projectName, projectDescription, projectBudget, ektronUploadedFilesIds);
        ...
        public static bool SaveSendAFileData(string centerId, string firstName, string lastName, string jobTitle, string companyName, string email, string phoneNumber, string projectName, string projectDescription, string projectQuantity, string ektronUploadedFilesIds, string externalFileLinks)    
        {
            try
            {
                //send email
                SendSendAFileInfoToCenter(centerId, firstName, lastName, jobTitle, companyName, email, phoneNumber, projectName, projectDescription, projectQuantity, ektronUploadedFilesIds, externalFileLinks);
        ...
        /// </summary>       
        public static void SaveUserSubscribeData(string centerId, string printType, string onlineType, string firstName, string lastName, string email, string address, string city, string state, string zipcode, string company, string phone)
        {
            try
            {
                //send email
                SendUserSubscribeDataInfoToCenter(centerId, printType, onlineType, firstName, lastName, email, address, city, state, zipcode, company, phone);
        ...
        /// <summary>
        /// This method is used to save the user request consultation data
        /// </summary>       
        public static void SaveUserRequestConsultationData(string centerId,  string firstName, string lastName, string email, string company, string phone, string howcanwehelp, bool isSignupChecked)
        {
            try
            {
                //send email
                SendUserConsultation_RequestDataInfoToCenter(centerId, firstName, lastName, email, company, phone, howcanwehelp,isSignupChecked);

                using (SqlConnection sqlConnection = new SqlConnection(adminToolConnectionString))
                {
                    string procName = "Add_New_User_Consultation_Request";
                    ...
                    using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                    {
                        ...
                        sqlCommand.Parameters.Add("@Email_Address", SqlDbType.NVarChar).Value = email;
                        sqlCommand.Parameters.Add("@Company", SqlDbType.NVarChar).Value = company;
                        sqlCommand.Parameters.Add("@Howcanwehelp", SqlDbType.NVarChar).Value = howcanwehelp;
                        sqlCommand.Parameters.Add("@IsSignupChecked", SqlDbType.NVarChar).Value = isSignupChecked;
                        sqlCommand.Parameters.Add("@Phone", SqlDbType.NVarChar).Value = phone;
                        sqlConnection.Open();
                        var reader = sqlCommand.ExecuteNonQuery();
                    }
        ...
        public static AboutCenter GetAboutUsContent(string centerId)
        {
            ...
            if (!string.IsNullOrEmpty(centerId))
            {
                try
                {
                    string cacheKey = "SirSpeedy-GetAboutUsContent" + centerId;
                    bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<Employee> GetAllEmployee(string fransId = "", bool refreshCache = false)
        {
            ...
            try
            {
                ...
                string cacheKey = "SirSpeedy-AllEmployeeDataFromdb" + fransId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static Employee GetEmployeeById(long employeeId, bool refreshCache = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-GetEmployeeDataFromdb" + employeeId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<JobPost> GetAllActiveJobs(bool refesh= false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-GetAllJobsFromCustomdb";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<Shop> GetAllShopsData(string centerId, bool refesh = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-GetAllShopsDataFromCustomdb" + centerId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static WhyWeAreDiff GetWhyWeAreDiffContent(string centerId, bool refresh = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-GetWhyWeAreDiffContent" + centerId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<Testimonials> GetAllTestimonials(string centerId, bool refresh = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-AllTestimonialsDataFromdb" + centerId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
                if (!dataExistInCache || refresh)
                {
                    using (SqlConnection sqlConnection = new SqlConnection(adminToolConnectionString))
                        ...
                        using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                        {
                            ...
                            while (reader.Read())
                            {
                                Testimonials t = new Testimonials()
                                {
                                    ...
                                    Statement = reader["Testimonial"] is DBNull ? null : reader["Testimonial"].ToString().Replace("''", "'"),
                                    ...
                                    IsAboutUs = reader["IsAboutUs"] is DBNull ? false : (reader["IsAboutUs"].ToString() == "1" ? true : false),
                                    IsManageIT = reader["IsManageIT"] is DBNull ? false : (reader["IsManageIT"].ToString() == "1" ? true : false),
                                    IsProjectExpert = reader["IsProjectExpert"] is DBNull ? false : (reader["IsProjectExpert"].ToString() == "1" ? true : false)
                                };
        ...
        public static List<Testimonials> GetAllTestimonials(bool refresh = false)
        {
            List<Testimonials> testimonialList = new List<Testimonials>();
            try
            {
                string cacheKey = "SirSpeedy-AllTestimonialsDataFromdbNational";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
                if (!dataExistInCache || refresh)
                {
                    using (SqlConnection sqlConnection = new SqlConnection(adminToolConnectionString))
                    {
                        string procName = "Get_All_Testimonials_From_Locals";
                        using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                        {
                            sqlCommand.CommandText = procName;
                            sqlCommand.CommandType = System.Data.CommandType.StoredProcedure;
                            ...
                            while (reader.Read())
                            {
                                Testimonials t = new Testimonials()
                                {
                                    ...
                                    TestimonialId = reader["id"] is DBNull ? 0 : int.Parse(reader["id"].ToString()),
                                    PicturePath = reader["Testimonial_Image"] is DBNull ? null : reader["Testimonial_Image"].ToString(),
                                    IsNational = reader["IsNational"] is DBNull ? false : (reader["IsNational"].ToString() == "1" ? true : false),
                                    IsAboutUs = reader["IsAboutUs"] is DBNull ? false : (reader["IsAboutUs"].ToString() == "1" ? true : false),
                                    IsManageIT = reader["IsManageIT"] is DBNull ? false : (reader["IsManageIT"].ToString() == "1" ? true : false),
                                    IsProjectExpert = reader["IsProjectExpert"] is DBNull ? false : (reader["IsProjectExpert"].ToString() == "1" ? true : false)
                                };
        ...
        public static List<Testimonials> GetAllActiveNationalTestimonials(bool refresh = false)
        {
            List<Testimonials> testimonialList = new List<Testimonials>();
            try
            {
                string cacheKey = "SirSpeedy-AllActive_TestimonialsDataFromdbNational";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
                if (!dataExistInCache || refresh)
                {
                    using (SqlConnection sqlConnection = new SqlConnection(adminToolConnectionString))
                    {
                        string procName = "Get_All_Active_Testimonials_For_National";
                        using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                        {
                            sqlCommand.CommandText = procName;
                            sqlCommand.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        public static List<Promotion> GetSelectedPromos(string centerId, bool refresh = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-SeletedPromoIdsDataFromdb" + centerId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<long> GetPSpageWhitePapersIds(string centerId, bool refresh = false)
        {
            ...
            try
            {
                string cacheKey = "SirSpeedy-PSpageWhitePapersIds" + centerId;
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
                if (!dataExistInCache || refresh)
                {
        ...
        private static SocialMedia GetFransSocialMediaData()
        {
            ...
            var getFransThirdPartyData = GetFransThirdPartyData();
        ...
        private static List<Partners> GetAllPartners()
        {
            ...
            try
            {
                ...
                string cacheKey = "SirSpeedy-AllPartnersDataFromdb";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
        ...
        public static List<FransData> GetAllCenterData(bool refreshCache = false)
        {
            List<FransData> centerList = new List<FransData>();
            try
            {
                string cacheKey = "SirSpeedy-AllCenterDataFromdb";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
                        ...
                        using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                        {
                            ...
                            while (reader.Read())
                            {
                                FransData d = new FransData()
                                {
                                    ...
                                    ClientLoginUrl = reader["Center_Client_Login_Url"] is DBNull ? null : reader["Center_Client_Login_Url"].ToString(),
                                    RequestConsultationEmail = reader["Center_RequestConsultation_Email"] is DBNull ? null : reader["Center_RequestConsultation_Email"].ToString(),
                                    ConsultationMessageId = reader["Consultation_Message_Id"] is DBNull ? -1 : Convert.ToInt32(reader["Consultation_Message_Id"]),
                                };
                                centerList.Add(d);
                            }
                        }
        ...
        /// </summary>        
        private static void SendRequestToQuoteInfoToCenter(string centerId, string firstName, string lastName, string jobTitle, string companyName, string email, string phoneNumber, string projectName, string projectDescription, string projectBudget, string ektronUploadedFilesIds)
        {
            ...
            if (!string.IsNullOrEmpty(centerId))
            {
                ...
                siteUrl = "http://" + domainName + "/" + centerId + "/Default.aspx";
            }
            else
            {
                ...
                siteUrl = "http://" + domainName + "/Default.aspx";
            }
            ...
            if (!string.IsNullOrEmpty(ektronUploadedFilesIds))
            {
                ...
                var ids = ektronUploadedFilesIds.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var id in ids)
                {
                    long assetId;
                    long.TryParse(id, out assetId);
                    if (assetId > 0)
                    {
                        var contentData = ContentHelper.GetContentById(assetId);
                        if (contentData != null && contentData.Id > 0)
                        {
                            sbEmailBody.Append("<a href=\"" + siteUrl + contentData.Quicklink + "\">" + contentData.Title + "</a><br/>");
                        }
                    }
                }
                sbEmailBody.Append("</td>");
            ...
            sbEmailBody.Append("<tr><td>ProjectBudget: </td><td>" + projectBudget + "</td></tr>");
            sbEmailBody.Append("</table>");

            Utility.SendEmail(adminEmail, email, email, sbEmailBody.ToString(), "Request a Quote");
        }

        /// <summary>
        /// helper method to build email data and send to center and copy user
        /// </summary>        
        private static void SendSendAFileInfoToCenter(string centerId, string firstName, string lastName, string jobTitle, string companyName, string email, string phoneNumber, string projectName, string projectDescription, string projectQuantity, string ektronUploadedFilesIds, string uploadExternalLinks)
        {
            string adminEmail = string.Empty;
            ...
            if (!string.IsNullOrEmpty(centerId))
            {
                var centerData = FransDataManager.GetFransData(centerId);
                adminEmail = centerData.Email;
                sendAFileEmail = centerData.Email;
                siteUrl = "http://" + domainName + "/" + centerId + "/Default.aspx";
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(ektronUploadedFilesIds))
            {
                ...
                foreach (var id in ids)
                {
                    ...
                    if (assetId > 0)
                    {
                        ...
                        if (contentData != null && contentData.Id > 0)
                        {
                            sbEmailBody.Append("<a href=\"" + "http://" + domainName + contentData.Quicklink + "\">" + contentData.Title + "</a><br/>");
                        }
                    }
                }
                sbEmailBody.Append("</td>");
            ...
            Utility.SendEmail(sendAFileEmail, email, adminEmail, sbEmailBody.ToString(), "New Send A File Submission");
        }
        ...
        private static void SendRegisterToCenter(string centerId, string firstName, string lastName, string jobTitle, string companyName, string email, string phoneNumber, string website)
        {
            ...
            if (centerData != null && !string.IsNullOrEmpty(centerData.Email))
            {
                ...
                Utility.SendEmail(centerEmail, email, email, sbEmailBody.ToString(), "A New Public User Registration To Your Center");
            }
        }
        ...
        private static void SendEmailApplicationJob(string centerId, string firstName, string lastName, string city, string stateName, string zipCode, string email, string coverNotes, string jobApplicationUrl, string resumeUrl)
        {
            ...
            if (centerData != null && !string.IsNullOrEmpty(centerData.JobApplicationEmail))
            {
                ...
                sbEmailBody.Append("<tr><td>Resume Copy: </td><td><a href=\"" + resumeUrl + "\">Download Resume</a></td></tr>");
                sbEmailBody.Append("</table>");

                Utility.SendEmail(centerEmail, email, "", sbEmailBody.ToString(), "Job Application");
		...
		private static void SendJobThankYouMessage(string firstName, string lastName, string email, string jobApplicationUrl)
        {
            ...
            if (contentData != null)
            {
                ...
                sbEmailBody.Append("<br/>");
                sbEmailBody.Append("Thank You<br/> TeamLogicIT.com");

                Utility.SendEmail(email, "NoReply@TeamLogicIT.com", "", sbEmailBody.ToString(), "Thank You");
            }
        }
        ...
        private static void SendContactUsInfoToCenter(string centerId, string firstName, string lastName, string userComments, string userEmail)
        {
            ...
            if (centerData != null && !string.IsNullOrEmpty(centerData.Email))
            {
                ...
                Utility.SendEmail(centerEmail, userEmail, "", sbEmailBody.ToString(), "Contact Us Request");
            }
        }

        private static void SendUserSubscribeDataInfoToCenter(string centerId, string printType, string onlineType, string firstName, string lastName, string email, string address, string city, string state, string zipcode, string company, string phone)
        {
            if (string.IsNullOrEmpty(centerId))
                return;
            ...
            if (centerData != null && !string.IsNullOrEmpty(centerData.SubscriptionEmail))
            {
                ...
                sbEmailBody.Append("<tr><td>Company: </td><td>" + company + "</td></tr>");
                sbEmailBody.Append("<tr><td>Phone: </td><td>" + phone + "</td></tr>");
                ...
                Utility.SendEmail(centerEmail, email, "", sbEmailBody.ToString(), "Subscribe Request");
            }
        }

        private static void SendUserConsultation_RequestDataInfoToCenter(string centerId, string firstName, string lastName, string email, string company, string phone, string howcanwehelp, bool isSignUpChecked)
        {
            if (string.IsNullOrEmpty(centerId))
                return;

            var centerData = GetFransData(centerId);
            if (centerData != null && !string.IsNullOrEmpty(centerData.RequestConsultationEmail))
            {
                var centerEmail = centerData.RequestConsultationEmail;
                StringBuilder sbEmailBody = new StringBuilder();

                sbEmailBody.Append("Hi,");
                sbEmailBody.Append("<br/>");
                sbEmailBody.Append("<p>Following data has been submitted through the Request Consultation form: </p>");
                sbEmailBody.Append("<table>");
                sbEmailBody.Append("<tr><td>FirstName: </td><td>" + firstName + "</td></tr>");
                sbEmailBody.Append("<tr><td>LastName: </td><td>" + lastName + "</td></tr>");                
                sbEmailBody.Append("<tr><td>Email: </td><td>" + email + "</td></tr>");
                ...
                sbEmailBody.Append("<tr><td>Company: </td><td>" + company + "</td></tr>");
                sbEmailBody.Append("<tr><td>Phone: </td><td>" + phone + "</td></tr>");
                sbEmailBody.Append("<tr><td>How can we help: </td><td>" + howcanwehelp + "</td></tr>");
                string signuptext = (isSignUpChecked) ? "Yes" : "No";
                sbEmailBody.Append("<tr><td>Marketing opt-in: </td><td>" + signuptext + "</td></tr>");
                sbEmailBody.Append("</table>");

                System.Threading.Thread emailThread = new System.Threading.Thread(delegate()
                {
                    Utility.SendEmail(centerEmail, email, "", sbEmailBody.ToString(), "Consultation Request",
                        "marketing@teamlogicit.com");

                    Utility.SendEmail("kausk@teamlogicit.com", email, "", sbEmailBody.ToString(),
        ...
        /// <summary>
        /// This method is used to get individual consultation text
        /// </summary>
        /// <param name="consultationId">consultationId</param>
        /// <returns></returns>
        public static RequestConsultationWording GetRequestConsultation(int consultationId)
        {
            if (default(int).Equals(consultationId)) return null;

            var fransid = GetFranchiseId();
            List<RequestConsultationWording> list = new List<RequestConsultationWording>();

            string cacheKey = "SirSpeedy-Get_All_ConsultationContent" + fransid;
            bool dataExistInCache = ApplicationCache.IsExist(cacheKey);

            if (!dataExistInCache)
            {
                list = AdminToolManager.GetAllRequestConsultationText(fransid);
                ApplicationCache.Insert(cacheKey, list, ConfigHelper.GetValueLong("smallCacheInterval"));
            }
            else
            {
                var cacheData = ApplicationCache.GetValue(cacheKey);
                if (cacheData != null)
                    list = (List<RequestConsultationWording>) cacheData;
            }

            RequestConsultationWording item =
                list.FirstOrDefault(x => x.Consultation_Id.Equals(consultationId));
            return (item);
