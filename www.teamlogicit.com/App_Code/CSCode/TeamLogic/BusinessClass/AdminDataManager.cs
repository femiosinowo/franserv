    ...
    public class AdminDataManager
    {
        ...
        public static SocialMedia GetNationalSocialMediaLinks(bool refreshCache = false)
        {
            ...
            if (contentId > 0)
            {
                try
                {
                    string cacheKey = "TeamLogic-GetNationalSocialMediaLinks" + contentId;
                    ...
                    if (!dataExistInCache || refreshCache)
                    {
                        ...
                        if (cData != null && !string.IsNullOrEmpty(cData.Html))
                        {
                            ...
                            foreach (var node in mediaNodes)
                            {
                                ...
                                else if (node.XPathSelectElement("mediaType").Value == "ITInflections" && node.XPathSelectElement("mediaImage/img") != null)
                                {
                                    if (node.XPathSelectElement("mediaLink/a") != null)
                                        data.ITInflectionsUrl = node.XPathSelectElement("mediaLink/a").Attribute("href").Value;
                                    data.ITInflectionsImgPath = node.XPathSelectElement("mediaImage/img").Attribute("src").Value;
                                }
                            }
                            if (data != null)
        ...
        /// <summary>
        /// This method is used to get National case studies content
        /// </summary>
        /// <returns></returns>
        public static List<ContentData> GetNationalHomePage_CaseStudiesContent(long smartFormId)
        {
            List<ContentData> dataList = null;
            long nationalCaseStudiesTaxId = ConfigHelper.GetValueLong("CaseStudiesNationalTaxId");
            if (nationalCaseStudiesTaxId > 0)
            {
                //get all items from taxonomy
                var taxItems = TaxonomyHelper.GetTaxonomyTree(nationalCaseStudiesTaxId, 1, true);
                List<long> cIds = new List<long>();
                if (taxItems != null && taxItems.TaxonomyItems != null)
                {
                    foreach (var tItem in taxItems.TaxonomyItems)
        ...
        public static FransData GetNationalCompleteAddress(FransData nationalData)
        {
            ...
            {
                string cacheKey = "SirSpeedy-NationalAddressData";
                bool dataExistInCache = ApplicationCache.IsExist(cacheKey);
