...
{    
            ...
            if (userApi.UserId > 0)
            {
                ...
                if (udata != null && udata.Id > 0)
                {
                    ...
                    if (userData != null)
                    {
                        ...
                        this.FillProjectExperts(centerId);
    ...
    protected void btnBanners_Click(object sender, EventArgs e)
    {
        if (Page.IsValid && hddnSelectedBanners.Value != "")
        {
            ...
            {
                var status = AdminToolManager.UpdateProjectExpertsIds(centerId, hddnSelectedBanners.Value);
                if (status > 0)
                {
                    this.FillProjectExperts(centerId);
    ...
    private void FillProjectExperts(string centerId)
    {
        var availableOurSolutionList = AdminToolManager.GetAvailableConsultingProject(centerId);
        if (availableOurSolutionList != null && availableOurSolutionList.Any())
        {
            ...
            if (workareaData != null && workareaData.ProjectExpertsContentIds != null && workareaData.ProjectExpertsContentIds.Any())
            {
                var existingBannerList = workareaData.ProjectExpertsContentIds;
                string ids = "";
