...
{    
            ...
            if (userApi.UserId > 0)
            {
                ...
                if (udata != null && udata.Id > 0)
                {
                    ...
                    if (userData != null)
                    {
                        ...
                        this.FillITSolutionContent(centerId);
    ...
    protected void btnBanners_Click(object sender, EventArgs e)
    {
        if (Page.IsValid && hddnSelectedBanners.Value != "")
        {
            ...
            {
                var status = AdminToolManager.UpdateITSolutionIds(centerId, hddnSelectedBanners.Value);
                if (status > 0)
                {
                    this.FillITSolutionContent(centerId);
                ...
                else
                {
                    ...
                    lblError.Text = "Sorry, an error has occured saving IT Solution data.";
                }
            }
        }
    }

    private void FillITSolutionContent(string centerId)
    {
        var availableITSolutionList = AdminToolManager.GetAvailableITSolutions(centerId);
        if (availableITSolutionList != null && availableITSolutionList.Any())
        {
            List<ContentData> availableBannersContent = new List<ContentData>();
            ContentCriteria cc = new ContentCriteria();
            cc.AddFilter(ContentProperty.Id, CriteriaFilterOperator.In, availableITSolutionList);
            var cList = ContentHelper.GetListByCriteria(cc);

            if (cList != null && cList.Any())
            {
                foreach (var cId in availableITSolutionList)
                {
            ...
            if (workareaData != null && workareaData.ITSolutionContentIds != null && workareaData.ITSolutionContentIds.Any())
            {
                var existingBannerList = workareaData.ITSolutionContentIds;
                string ids = "";
