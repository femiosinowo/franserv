...
public partial class AdminTool_Templates_manage_all_quotes : System.Web.UI.Page
{
    ...
    {
        var allQuotesData = AdminToolManager.GetAllRequestToQuotes(centerId);
        if (ViewState["CurrentAlphabet"] != null)
        {
            ...
            if (selectedAlphabet != "ALL")
            {
                ...
                var filteredList = allQuotesData.Where(x => x.FirstName.ToLower().StartsWith(selectedAlphabet)).ToList();
                GridView1.DataSource = filteredList;
                GridView1.DataBind();
            }
            else
            {
                GridView1.DataSource = allQuotesData;
                GridView1.DataBind();
            }
        }
    }
