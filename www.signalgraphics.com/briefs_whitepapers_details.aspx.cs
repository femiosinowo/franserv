...
public partial class briefs_whitepapers_details : System.Web.UI.Page
{
    ...
    private DataTable GetBriefsWHitepaperContent()
    {
        ...
        string cacheKey = String.Format("SignalGraphics:Briefswhitepapersdetails:GetBriefsWhitepaperContent:Folderid={0}:ContentID={1}:SmartFormId={2}:FranchiseId={3}", folderID, contentId, SmartFormXMLConfig, fransId);
    ...
    private void LoadPrimaryBusiness()
    {
        try
        {
            string cacheKey = String.Format("SignalGraphics:briefs_whitepapers_details_:LoadPrimaryBusiness");
    ...
    private void LoadMarketingEmployee()
    {
        try
        {
            string cacheKey = String.Format("SignalGraphics:briefs_whitepapers_details_:LoadMarketingEmployee");
            DataTable subjects = CacheBase.Get<DataTable>(cacheKey);
            if (subjects == default(DataTable) || subjects.Rows.Count == 0)
            {
                subjects = new DataTable();
