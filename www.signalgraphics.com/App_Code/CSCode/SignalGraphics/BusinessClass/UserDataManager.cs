...
{
    /// <summary>
    /// Summary description for SignalGraphics UserManager
    /// </summary>
    public class UserDataManager
    {
        ...
        public static CustomUserData getUserInfoData(string emailAdd)
        {
            ...
            if (!string.IsNullOrEmpty(emailAdd))
            {
                try
                {
                    string cacheKey = "SignalGraphics-GetUserData" + emailAdd;
            ...
            if (contentId > 0)
            {
                ...
                if (cData != null && !string.IsNullOrEmpty(cData.Html))
                {
                    try
                    {
                        ...
                        if (xDoc.XPathSelectElement("/root/MarketingTango/url/a") != null && xDoc.XPathSelectElement("/root/MarketingTango/image/img") != null)
                        {
                            data.MarketingTangoUrl = xDoc.XPathSelectElement("/root/MarketingTango/url/a").Attribute("href").Value;
                            data.MarketingTangoImgPath = xDoc.XPathSelectElement("/root/MarketingTango/image/img").Attribute("src").Value;
                        }
                    }
                    catch(Exception ex)
                    {
