...
public partial class portfolio : PageBase
{
    ...
    private static string PhotoHtml(string sId)
    {
        ...
        string cacheKey = String.Format("SignalGraphics:portfolio:PhotoHtml:Sid={0}:FranchiseId={1}",
            sId, fransId);
        ...
        if (String.IsNullOrWhiteSpace(html))
        {
            ...
            if (photoDataList != null && photoDataList.Any())
            {
                ...
                foreach (var photo in photoDataList)
                {
                    ...
                    if (i >= 0)
                    {
                        sbHtml.Append("<div id=\"content-" + photoData.PhotoId +
                                      "\" class=\"cs_image_content\"><a class=\"fancybox\" data-content-id=\"content-" +
                                      photoData.PhotoId + "\" rel=\"group-" + sId + "\" href=\"" + photoData.LargeUrl +
                                      "\">" + photoData.Title + "</a>");
                        sbHtml.Append("<div class=\"cs_image_content_desc_lightbox\"><h3>" + photoData.Title +
                                      "</h3>&nbsp;<p>" + photoData.Description + "</p></div></div>");
                    }
                    i++;
                }
                ...
                if (firstImageData != null)
                {
                    ...
                    sbHtml.Append("<div id=\"content-" + firstPhotoData.PhotoId +
                                  "\" class=\"cs_image_content\"><a class=\"fancybox\" data-content-id=\"content-" +
                                  firstPhotoData.PhotoId + "\" rel=\"group-" + sId + "\" href=\"" + firstPhotoData.LargeUrl +
                                  "\">" + firstPhotoData.Title + "</a>");
                    sbHtml.Append("<div class=\"cs_image_content_desc_lightbox\"><h3>" + firstPhotoData.Title +
                                  "</h3>&nbsp;<p>" + firstPhotoData.Description + "</p></div></div>");
                }
    ...
    {
        string fransId = FransDataManager.IsFranchiseSelected() ? FransDataManager.GetFranchiseId() : "0";
        string cacheKey = String.Format("SignalGraphics:portfolio:GetFirstImageOfAlbum:Sid={0}:FranchiseId={1}",
            sId, fransId);
    ...
    {
        string fransId = FransDataManager.IsFranchiseSelected() ? FransDataManager.GetFranchiseId() : "0";
        string cacheKey = String.Format("SignalGraphics:portfolio:GetFirstImageTitleDesp:Sid={0}:FranchiseId={1}",
            sId, fransId);
        ...
        if (String.IsNullOrWhiteSpace(html))
        {
            ...
            if (photoDataList != null && photoDataList.Any())
            {
                ...
                {
                    var firstPhotoData = FlickrManager.GetFlickrPhotoInfo(firstImageData.PhotoId);
                    html = "<div class=\"cs_image_content_desc_lightbox\"><h3>" + firstPhotoData.Title +
                           "</h3>&nbsp;<p>" + firstPhotoData.Description + "</p></div>";
                }
