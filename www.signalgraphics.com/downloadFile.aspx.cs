    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(Request.QueryString["filepath"]))
        {
            try
            {
                ...
                {
                    //to support files have a special characters like &.
                    string path = HttpUtility.UrlDecode(Request.RawUrl);
                    path = path.Substring(path.IndexOf("filepath=") + 9);
                    //string path = Request.QueryString["filepath"];
                    using (new Impersonator(sharedDriveUserName, sharedDriveDomain, sharedDrivePassword))
                    {
                        if (File.Exists(path))
                        {
                            var allFileBytes = File.ReadAllBytes(path);
