...
public partial class login_national : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (HttpContext.Current.Session["userCenterId"] != null && !String.IsNullOrWhiteSpace(HttpContext.Current.Session["userCenterId"].ToString()))
            {
                selectedCenter.Value = HttpContext.Current.Session["userCenterId"].ToString();
            }
            else if (Request.QueryString["centerId"] != null)
            {
                selectedCenter.Value = Request.QueryString["centerId"];
            }
            else
            {
    ...
    protected void Upload_NationalComplete(object sender, AjaxControlToolkit.AjaxFileUploadEventArgs e)
    {
        ...
        {
            HttpContext.Current.Session["fileSize_" + e.FileId] = e.FileSize;
            HttpContext.Current.Session["fileContents_" + e.FileId] = e.GetContents();
            HttpContext.Current.Session["fileExtension_" + e.FileId] = e.FileName.Substring(e.FileName.LastIndexOf('.'));
            HttpContext.Current.Session["fileName_" + e.FileId] = e.FileName;
        }
    }
    ...
    private void GetCenterDetails()
    {
        if (!string.IsNullOrEmpty(Request.QueryString["centerId"]))
        {
            ...
            if (centerLocationList != null && centerLocationList.Any())
            {
                ...
                {
                    ltrCenterAddress.Text = centerDetails.Address1 + "<br />" + centerDetails.City + ", " + centerDetails.State + " " + centerDetails.Zipcode;
