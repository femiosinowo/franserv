...
public partial class about_national : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            //this is added globally to all pages via master page now.
            //if (Request.QueryString.Get("id") != null)
            //{
            //    long cid = Convert.ToInt64(Request.QueryString.Get("id"));
            //    Ektron.Cms.API.Metadata mapi = new Ektron.Cms.API.Metadata();
            //    CustomAttributeList attributeList = new Ektron.Cms.CustomAttributeList();
            //    attributeList = mapi.GetContentMetadataList(cid);
            //    if (attributeList["EloquaTrackingScripts"].DoesAtrributeExist &&
            //        attributeList["EloquaTrackingScripts"].Value != null)
            //        Literal1.Text = Server.HtmlDecode(Convert.ToString(attributeList["EloquaTrackingScripts"].Value));
            //}
        }
    }   
        ...
        if (contents != null && contents.Count > 0)
        {
            ...
            if (contentData != null && contentData.Html != string.Empty)
            {
                try
                {
                    ...
                    string logoImgPath = cbGoogleInfoBoxImgLogo.EkItem != null ? cbGoogleInfoBoxImgLogo.EkItem.Html : "<img alt=\"Sir Speedy\" src=\"/images/logo-infobox.png\"/>";
