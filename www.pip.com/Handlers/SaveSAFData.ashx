        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(hdnFileIDs))
            {
                ...
                if (arrFileIDs != null && arrFileIDs.Length > 0)
                {
                    ...
                    foreach (string strFileID in arrFileIDs)
                    {
                        var fileData = (byte[])HttpContext.Current.Session["fileContents_" + strFileID];
                        var fileName = HttpContext.Current.Session["fileName_" + strFileID] as string;
                        var fileExtension = (string)HttpContext.Current.Session["fileExtension_" + strFileID];
                        ...
                        if (fileData != null && fileExtension != null && fileName != null)
                        {
                            try
                            {
                                ...
                                using (new Impersonator(sharedDriveUserName, sharedDriveDomain, sharedDrivePassword))
                                {
                                    ...
                                    ektronUploadedFilesIds += "<a target=\"_blank\" href=\"" + encodedURL + "\">" + fileName + "</a>, ";
                                }
            ...
            bool isSaved = FransDataManager.SaveSendAFileData(selectedCenter, firstName, lastName, jobTitle, companyName, emailAddress, phoneNumber, projectName, projectDescription, projectQuantity, ektronUploadedFilesIds, externalFileLinks, projectDueDate);
            if (!isSaved)
            {
            ...
            {
                string formattedLinks = ektronUploadedFilesIds;
