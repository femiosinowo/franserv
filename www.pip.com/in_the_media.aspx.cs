...
public partial class in_the_media : System.Web.UI.Page
{
    ...
    {
        cbMediaSideContent.DefaultContentID = ConfigHelper.GetValueLong("MediaSideContentID");
        cbMediaSideContent.CacheInterval = ConfigHelper.GetValueLong("longCacheInterval");
        cbMediaSideContent.Fill();
    }
    ...
    {        
        if (!Page.IsPostBack)
        {
            ...
            {
                Master.BodyClass += " local-media-link media ";
                desktopNavLocal.Visible = true;
                desktopNavNational.Visible = false;
            }
            else
            {
                Master.BodyClass += " media-link media ";
