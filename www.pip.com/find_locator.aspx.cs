...
{    
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //Literal1.Text = " <script type='text/javascript'>init_map('my_map',33.976463,-118.036812,14,'location_map');</script>";
        }
    }
    ...
    protected void ddlCountryList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "appendLocationId", "<script type=\"text/javascript\">window.location.href = \"/find_locator.aspx#all_locations\";</script>", false);
    }
    ...
    private void GetLocationsForCountry(string countryCode)
    {
        if(countryCode != string.Empty)
        {
            ...
            if(allLocationData != null && allLocationData.Any())
            {
                ...
                {
                    //countryList = countryList.OrderBy(x => x.State).ToList();
                    //var filteredList = from state in countryList
                    //                   group state by state.StateFullName into StateGroup
                    //                   from CityGroup in
                    //                       (from state in StateGroup
                    //                        group state by state.City)
                    //                   group CityGroup by StateGroup.Key;
                     ...
                     if (stateGroupList != null)
                    {
                        ...
                        {                           
                            ...
                            {                                          						
                                if (city.FransId != "missionviejoca002")
                                {							
                                   var cityData = city;
