...
public partial class briefs_whitepapers_details : System.Web.UI.Page
{
    ...
    {
        cbBriefWhitepapersDetailsHeaderImage.DefaultContentID = ConfigHelper.GetValueLong("WhitePapersDetailPageBannerImgCId");
        cbBriefWhitepapersDetailsHeaderImage.CacheInterval = ConfigHelper.GetValueLong("longCacheInterval");
        cbBriefWhitepapersDetailsHeaderImage.Fill();
    ...
    private DataTable GetBriefsWHitepaperContent()
    {
        ...
        string cacheKey = String.Format("Pip:Briefswhitepapersdetails:GetBriefsWhitepaperContent:Folderid={0}:ContentID={1}:SmartFormId={2}:FranchiseId={3}", folderID, contentId, SmartFormXMLConfig, fransId);
    ...
    private void LoadPrimaryBusiness()
    {
        try
        {
            string cacheKey = String.Format("PIP:briefs_whitepapers_details_:LoadPrimaryBusiness");
    ...
    private void LoadMarketingEmployee()
    {
        try
        {
            string cacheKey = String.Format("PIP:briefs_whitepapers_details_:LoadMarketingEmployee");
            DataTable subjects = CacheBase.Get<DataTable>(cacheKey);
            if (subjects == default(DataTable) || subjects.Rows.Count == 0)
            {
                subjects = new DataTable();
    ...
    protected void btnDownloadNow_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(adminToolConnectionString))
                {
                    ...
                    using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                    {
                        ...
                        sqlCommand.Parameters.Add("@Zipcode", SqlDbType.NVarChar).Value = zip;
                        sqlConnection.Open();
            ...
            if (isSaved)
            {
                ...
                //send email
                SendWhitepapersDownloadEmail(centerId, firstName, lastName, email, companyWebsite, zip, ddlPrimaryBusiness.SelectedItem.Text, ddlMarketingEmployee.SelectedItem.Text, pdfPath);
