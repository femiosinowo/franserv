    ...
    {        
        ...
        {            
            ...
            if (password.Equals(password_confirm))
            {
                ...
                long cmsUserId = CommunityUserHelper.AddCmsMembershipUser(firstName, lastName, email, password, email, selectedCenterData.FransId);
                if (cmsUserId > 0)
                {
                ...
                if (isSaved && isCMSUserSaved)
                {
                    ...
                    Session.Add("userFirstName", firstName);
                    Session.Add("userLastName", lastName);
                    Session.Add("userJobTitle", jobTitle);
                    Session.Add("userCompanyName", companyName);
                    Session.Add("userPhoneNumber", phoneNumber);
                    Session.Add("userCenterId", selectedCenter);
                    Session.Add("userWebsite", companyWebsite);
                    Session.Add("externalLogin", "false");

                    var fransId = selectedCenterData.FransId;
