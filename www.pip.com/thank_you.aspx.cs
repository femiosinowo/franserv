...
public partial class thank_you : PageBase
{
        ...
        if (!string.IsNullOrEmpty(centerId))
        {
            ...
            if (centerLocationList != null && centerLocationList.Any())
            {
                ...
                if (centerDetails != null)
                {
                    ...
                    sb.Append("<ul>");
                    sb.Append("<li id=\"location_address\">" + centerDetails.Address1 + "</li>");
                    sb.Append("<li id=\"location_address2\">"+ centerDetails.City + ", " + centerDetails.State + " " + centerDetails.Zipcode+"</li>");
                    sb.Append("<li id=\"location_phone\">" + centerDetails.PhoneNumber + "</li>");
                    sb.Append("<li class=\"contact-icon-fax\">" + centerDetails.FaxNumber + "</li>");
                    sb.Append("<li id=\"location_email\"><a href=\"mailto:" + centerDetails.Email + "\">" + centerDetails.Email + "</a></li>");
                    sb.Append("</ul>");
                    ltrCenterAddress.Text = sb.ToString();
    ...
    protected void btnLogout_Click(object sender, EventArgs e)
    {
        ...
        if (selectedCenter == "" && !string.IsNullOrEmpty(Request.QueryString["centerId"]))
        {
            selectedCenter = Request.QueryString["centerId"];
        }

        ClearUserSession();

        var fransId = FransDataManager.GetFranchiseId();
        string url = "/thank-you/?type=logout&centerId=" + selectedCenter;
        if (!string.IsNullOrEmpty(fransId))
            url = "/" + fransId + "/thank-you/?type=logout&centerId=" + selectedCenter;
        ...
        if (!string.IsNullOrEmpty(path))
        {
            try
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    context.Response.AppendHeader("content-length", allFileBytes.Length.ToString());
                    context.Response.ContentType = GetMimeType(fileName);
                    context.Response.BinaryWrite(allFileBytes);
