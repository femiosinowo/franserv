...
public partial class UserControls_MainNav : UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (FransDataManager.IsFranchiseSelected())
            {
                ...
                pnlNationalFindLocation.Visible = false;
                pnlLocalContactUs.Visible = true;
            }
            else
            {
                ...
                pnlNationalFindLocation.Visible = true;
                pnlLocalContactUs.Visible = false;
    ...
    {
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetPSMMContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        ...
                        string xml = (contentXML.SelectSingleNode("/root/imageSlider").InnerXml == null) ? String.Empty : contentXML.SelectSingleNode("/root/imageSlider").InnerXml;
                        ...
                        xml = (contentXML.SelectSingleNode("/root/url").InnerXml == null) ? String.Empty : contentXML.SelectSingleNode("/root/url").InnerXml;
                        string url = "#";
                        if (!String.IsNullOrEmpty(xml))
                        {
                            url = Regex.Match(xml, "<a.+?href=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
                        }
                        counter++;
    ...
    private DataTable GetPSMMSideContent()
    {
        ...
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetPSMMSideContent:FranchiseId={0}",
            _fransId);
    ...
    {
         string cacheKey = String.Format("Pip:UserControls_MainNav:GetCompanyInfoContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        ...
                        string xml = (contentXML.SelectSingleNode("/root/url").InnerXml == null) ? String.Empty : contentXML.SelectSingleNode("/root/url").InnerXml;
                        string url = "#";
                        if (!String.IsNullOrEmpty(xml))
                        {
                            url = Regex.Match(xml, "<a.+?href=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
        ...
        if(contents != null && contents.Any())
        {
            ...
            foreach (Ektron.Cms.ContentData contentData in contents)
            {
                try
                {
                    contentXML.LoadXml(contentData.Html);
                    XmlNodeList xnList = contentXML.SelectNodes("/root");
                    if (xnList[0]["CompanyInfoImage"] != null)
                    {
                        var xml = xnList[0]["CompanyInfoImage"].InnerXml;
    ...
    {
         string cacheKey = String.Format("Pip:UserControls_MainNav:GetManagementTeamContent:FranchiseId={0}",
            _fransId);
    ...
    {
       string cacheKey = String.Format("Pip:UserControls_MainNav:GetManagementTeamContentForLocal:FranchiseId={0}",
            _fransId);
    ...
    private DataTable GetNewsContent()
    {
        ...
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetNewsContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        contentXML.LoadXml(contentData.Html);
                        XmlNodeList xnList = contentXML.SelectNodes("/root");
                        string title = String.IsNullOrEmpty(xnList[0]["title"].InnerXml) ? String.Empty : xnList[0]["title"].InnerXml;
                        //if (title.Length >= 50)
                        //{
                        //    title += "...";
                        //}
                        ...
                        string date = String.IsNullOrEmpty(xnList[0]["date"].InnerXml) ? String.Empty : xnList[0]["date"].InnerXml;
                        string city = String.IsNullOrEmpty(xnList[0]["city"].InnerXml) ? String.Empty : xnList[0]["city"].InnerXml;
                        string state = String.IsNullOrEmpty(xnList[0]["state"].InnerXml) ? String.Empty : xnList[0]["state"].InnerXml;
                        string content = String.IsNullOrEmpty(xnList[0]["content"].InnerXml) ? String.Empty : xnList[0]["content"].InnerXml;

                        DateTime tempdate = Convert.ToDateTime(date);
                        string newFormattedDate = tempdate.ToString("MMMM dd, yyyy");

                        string xml = (contentXML.SelectSingleNode("/root/image").InnerXml == null) ? String.Empty : contentXML.SelectSingleNode("/root/image").InnerXml;
                        string imgSRC = String.Empty;
                        if (!String.IsNullOrEmpty(xml))
                        {
                            imgSRC = Regex.Match(xml, "<img.+?src=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
                        ...
                        DTSource.Rows.Add(titleReformatted, date, newFormattedDate, city, state, content, imgSRC, url, hrefText, counter);
                    }
                    catch (Exception ex)
                    {
                        Log.WriteError(ex);
    ...
    {
         string cacheKey = String.Format("Pip:UserControls_MainNav:GetPartnersContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        ...
                        {
                            imgSRC = Regex.Match(xml, "<img.+?src=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
                            alt = Regex.Match(xml, "<img.+?alt=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
                        }
                        ...
                        DTSource.Rows.Add(imgSRC, alt, url, dateCreated, cssClass, counter);
                    }
                    catch (Exception ex)
                    {
                        Log.WriteError(ex);
    ...
    {
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetInTheMediaContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        ...
                        XmlNodeList xnList = contentXML.SelectNodes("/root");
                        string title = xnList[0]["title"].InnerXml;
                        string source = xnList[0]["sourceName"]!=null? xnList[0]["sourceName"].InnerXml:"";
    ...
    {
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetBriefsWHitepaperContent:FranchiseId={0}",
            _fransId);
            ...
            if (contents != null && contents.Count > 0)
            {
                foreach (Ektron.Cms.ContentData contentData in contents)
                {
                    try
                    {
                        ...
                        DTSource.Rows.Add(title, teaserMM, abstractText, content, url, hrefText, imgSRC, dateCreated, counter);
                    }
                    catch (Exception ex)
                    {
                        Log.WriteError(ex);
    ...
    {
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetCaseStudiesContent:FranchiseId={0}:Count={1}",
            _fransId, count);
    ...
    {
        string cacheKey = String.Format("Pip:UserControls_MainNav:GetJoinOurTeamContent:FranchiseId={0}",
            _fransId);
    ...
    private void GetMenuFirstLevel()
    {
        ...
        var mainNavData = MenuHelper.GetMenuTree(mainNavId);
